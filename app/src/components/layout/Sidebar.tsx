...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const NavLink = ({
    ...
    return (
      <Link
        href={href}
        className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 ${isActive ? "bg-primary-800 text-cream-100 shadow-md" : "text-primary-600 hover:bg-cream-200 hover:text-primary-800"}`}
      >
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-16 left-0 bottom-0 w-64 bg-cream-100 border-r border-cream-300 transform transition-transform duration-300 ease-in-out z-30 overflow-y-auto scrollbar-thin scrollbar-thumb-cream-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div className="p-4 space-y-6">
          <nav>
            <p className="px-4 text-xs font-semibold text-primary-400 uppercase tracking-wider mb-3">
              Navigation
            </p>
          ...
          {!authLoading && isAdmin && (
            <>
              <nav>
                ...
                {adminStats && (
                  <div className="mt-4 bg-cream-200 rounded-xl p-4 space-y-4">
                    {/* System Status */}
                    <div className="space-y-2">
                      <p className="text-xs font-semibold text-primary-500 uppercase">Etat Systeme</p>
                    ...
                    {/* Key Metrics */}
                    <div className="space-y-2">
                      <p className="text-xs font-semibold text-primary-500 uppercase">Documents</p>
                    ...
                    {/* Activity */}
                    <div className="space-y-2">
                      <p className="text-xs font-semibold text-primary-500 uppercase">Activite</p>
                      <div className="space-y-1">
                    ...
                    <Link href="/admin" className="block w-full text-center py-2 bg-primary-700 hover:bg-primary-800 text-cream-100 text-sm font-medium rounded-lg transition-colors">
          ...
          <div className="px-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-cream-200 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-primary-800">{formatNumber(stats.documents)}</p>
                <p className="text-xs text-primary-500">Documents</p>
              </div>
              <div className="bg-cream-200 rounded-lg p-3 text-center">
                <p className="text-2xl font-bold text-primary-800">{stats.divisions}</p>
